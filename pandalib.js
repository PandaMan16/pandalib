...
const panda = {
  ...
  createToastMessage: function (title, type, message) {
    let notificationsObject = document.getElementById('notifications');
    let icon, color;
    switch (type) {
      case 'success':
      ...
      case 'error':
        ...
        color = '#ff3860'; // Exemple de couleur pour 'error'
        break;
      case 'info':
        icon = 'fa-solid fa-circle-info';
        color = '#209cee'; // Exemple de couleur pour 'info'
        break;
      case 'warning':
        icon = 'fa-solid fa-triangle-exclamation';
        color = '#ffdd57'; // Exemple de couleur pour 'warning'
        break;
      default:
        return;
    }
    ...
    newtoast.innerHTML = `
            <div class="toast ${type}" style="border-left-color: ${color};">
                <i class="${icon}" style="color: ${color};"></i>
                <div>
                    <div class="title">${title}</div>
                    <span>${message}</span>
                </div>
                <i onclick="(this.parentElement).remove()" class="fa-solid fa-xmark"></i>
            </div>
        `;
    notificationsObject.appendChild(newtoast);
...
const nopub = ['*.panda', 'pandtown.fr'];

// Trouver une correspondance en utilisant location.hostname
// trouve une correspondance dans le tableau nopub avec location.hostname prise en compte dans la recherche des wildcard
const found = nopub.find((obj) => {
  let regex = new RegExp(obj.replace(/\./g, "\\.").replace(/\*/g, ".*"));
